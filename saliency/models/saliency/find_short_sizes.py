...
def find_max_otro(path):
    maxh = 0
    maxw = 0
    fnf = ['finalistas', 'no_finalistas']
    for f in fnf:
        path_fin = os.path.join(path, f)
        videos = os.listdir(path_fin)
        for v in videos:
            path_video = os.path.join(path_fin, v)
            print(path_video)
            saliency = pd.read_csv(path_video, index_col=0, thousands=',', engine='python')
            saliency = saliency.drop(['0', '1'], axis=1)
            salarr = saliency.to_numpy()
            sh = salarr.shape[0]
            sw = salarr.shape[1]
...
path_train = '/home/aitorgalan/Escritorio/tfm-impacto-youtube-cortos/saliency/models/saliency/short_saliency/train'
path_test = '/home/aitorgalan/Escritorio/tfm-impacto-youtube-cortos/saliency/models/saliency/short_saliency/test'

find_max_otro(path_train)
find_max_otro(path_test)
